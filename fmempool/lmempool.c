...
static
byte*	f_create_map(){
	byte* pmap = (byte*)f_malloc(sizeof(byte) * FMEM_PAGE_SIZE8);
...
free_block*	f_block_create(int index, size_t fb_size, size_t total_size){
	char* fb = (char*)f_malloc(total_size);
	char* iter = fb;
...
free_list*	fl_create(int index, size_t len, int is_create_fb){
	...
	free_list*	fl = (free_list*)f_malloc(sizeof(free_list));
...
fl_mgr*	flmgr_create(int is_create_fb, int is_create_timer){
	fl_mgr* mgr = (fl_mgr*)f_malloc(sizeof(fl_mgr));
...
static inline
void	f_mp_create(){
    fmem_pool* pmem_t = (fmem_pool*)f_malloc(sizeof(fmem_pool));
    ...
    //pthread_key_create(&pmem_t->key, NULL);
    pmem = pmem_t;
...
void	f_steal_from_center(free_list* fl, int idx){
	...
	if( head ){
		...
		c_fl->count -= 1;
		for(i=1; i<FMEM_STEAL_COUNT && tail->block_data.next; ++i, c_fl->count -= 1)
			tail = tail->block_data.next;
...
void*	_f_alloc_fromsys(size_t size){
	sys_block* block = (sys_block*)malloc(size + sizeof(size_t) + sizeof(fb_head));
	block->len = size;
...
void*	f_alloc(size_t size){
	...
	fl_mgr*	flmgr = (fl_mgr*)pthread_getspecific(pmem->key);
	if( !flmgr ){
		flmgr = flmgr_create(FMEM_NOT_CREATE_FB, FMEM_CREATE_TR);	// the thread cache get from center
...
void*	_f_realloc_bysys(void* sys_data, size_t size){
	sys_block* block = (sys_block*)realloc(sys_data, size + sizeof(size_t) + sizeof(fb_head));
	block->len = size;
	block->fb.b_head.idx = FMEM_SYS_ALLOC_IDX;
...
/*
// gcc -Wall -lpthread -o testmem lmempool.c lmutex.c lhash.c list.c
int	main(int argc, char** argv)
{
	int hsize = sizeof(fb_head);
	int tsize = sizeof(free_block);

	printf("hsize=%d, tsize=%d\n", hsize, tsize);
	return 0;
}
*/
