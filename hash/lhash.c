...
static inline
int		hash_str(f_hash* phash, char* key)
{
...
char*	make_key(char* key)
{
	...
	char* pkey = (char*)malloc(len);
	return strncpy(pkey, key, len);
}

static inline
f_hash_node*	make_node(f_hash_node* pnode, int type, void* key, void* value)
{
...
static
f_hash_node*	hash_find(f_hash* phash, int h, int type, void* key)
{
...
static
void	hash_add(f_hash* phash, int h, int type, void* key, void* value)
{
...
static
void 	hash_set(f_hash* phash, int h, int type, void* key, void* value)
{
...
static inline
void*	hash_get(f_hash* phash, int h, int type, void* key)
{
	f_hash_node* pnode = hash_find(phash, h, type, key);
	if( pnode ){
		return pnode->value;
...
static inline
void*	hash_del(f_hash* phash, int h, int type, void* key)
{
	f_hash_node* node = hash_get_list(phash, h);
	if( !node ) return NULL;
	int size = hash_get_size(phash, h);
...
void	hash_delete(f_hash* phash)
{
	...
	for(i=0; i<size; ++i)
	{
		f_hash_node* pmgr = hash_get_list(phash, i);
		int list_size = hash_get_size(phash, i);
		f_hash_node* node = pmgr;
		int i;
...
inline
void	hash_set_str(f_hash* phash, char* key, void* value)
{
...
inline
void*	hash_get_str(f_hash* phash, char* key)
{
	int h = hash_str(phash, key);
...
void*	hash_del_str(f_hash* phash, char* key)
{
	int h = hash_str(phash, key);
